...
const FormUpdateContactInfo = (props) => {
  ...
  const [email, setEmail] = useState(
    accountInformation.Email ? accountInformation.Email : ""
  );
  const [phone, setPhone] = useState(
    accountInformation.Phone ? accountInformation.Phone : ""
  );
  const [address, setAddress] = useState(
    accountInformation.Address ? accountInformation.Address : ""
  );
  ...
  const onSubmit = async () => {
    ...
    if (!isEmail(email)) {
      pushSnackbarAction("warning", "Invalid Email format");
    } else if (phone !== "" && !isPhoneNumber(phone)) {
      pushSnackbarAction("warning", "Invalid phone number format.");
    } else {
