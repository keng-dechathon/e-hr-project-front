          ...
          {SidebarData.map((item, index) => {
            return (
              <List key={index} className={classes.list}>
                {Role !== "Staff" ? (
                  open ? (
                    <div className={classes.listTopic}>{item.title}</div>
                  ) : (
                    item.title !== "Main" && <Divider />
                  )
                ) : open ? (
                  <div className={classes.listTopic}>
                    {item.title !== "Management" ? item.title : ""}
                  </div>
                ) : (
                  item.title !== "Main" &&
                  item.title !== "Management" && <Divider />
                )}
                {item.subNav.map((subItem, index1) => {
                  if (Role) {
                    return subItem.role.map((role) => {
                      if (role == Role) {
