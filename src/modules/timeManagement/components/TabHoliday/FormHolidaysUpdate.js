...
const FormHolidaysUpdate = (props) => {
    ...
    const item = Object.keys(holidaysInformation).length !== 0 && option != 'add' ? holidaysInformation.data.filter(value => value.ID === id) : ''
    const [holidayID, setHolidayID] = useState(item.length !== 0 ? item[0].ID ? item[0].ID : '' : '')
    const [name, setName] = useState(item.length !== 0 ? item[0].Holiday_Name ? item[0].Holiday_Name : '' : '')
    ...
    useEffect(() => {
        setTimeout(() => setUser({ ID: holidayID, Holiday_Name: name, begin_date: moment(start).format('yyyy-MM-DD'), end_date: moment(end).format('yyyy-MM-DD') }))
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns}>
            ...
            >
                ...
                >
                    ...
                    <Grid item xs={12} >
                        <InputLabel><pre> Date ( Begin to End )*</pre></InputLabel>
                    </Grid>
                    <Grid item xs={12} style={{ display: 'flex', paddingTop: '0px' }}>
                        ...
                        <DesktopDatePicker
                            inputFormat="dd/MM/yyyy"
                            value={end}
                            onChange={(e) => {setEnd(new Date(moment(e).format())) }}
                            renderInput={(params) => <TextField required {...params} />}
                        />
                    </Grid>
                </Grid>
