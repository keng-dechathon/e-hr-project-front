...
import FormPositionManagement from "./FormPositionManagement";
...
const CardPositionManagement = () => {
  ...
  useEffect(() => {
    if (deleteID !== "" && confirmDelete) {
      const onDelete = async (id) => {
        await deletePosition([String(id)]);
        dispatch(getAllPositionInformtion());
        setDeleteID("");
  ...
  const setDataGrid = () => {
    if (Object.keys(AllPositionInformation).length !== 0) {
      AllPositionInformation.data.map((item, index) => {
        Info.push(item);
  ...
  return (
    <>
      <ConfirmDialog
        ...
        message={"Do you insist on deleting Company ?"}
      />
      <ModalUpdate
        open={open}
...
export default CardPositionManagement;
