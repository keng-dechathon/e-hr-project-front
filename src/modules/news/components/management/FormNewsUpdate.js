...
const FormNewsUpdate = (props) => {
  ...
  const item =
    Object.keys(allNewsInformation).length !== 0 && option != "add"
      ? allNewsInformation.data.filter((value) => value.News_id === id)
      : "";
  const [newsId, setNewsID] = useState(
    item.length !== 0 ? (item[0].News_id ? item[0].News_id : "") : ""
  );
  const [topic, setTopic] = useState(
    item.length !== 0 ? (item[0].Topic ? item[0].Topic : "") : ""
  );
