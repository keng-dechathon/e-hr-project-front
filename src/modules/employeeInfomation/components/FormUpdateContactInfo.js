...
const FormUpdateContactInfo = (props) => {
    ...
    const [email, setEmail] = useState(empInformationByID.Email ? empInformationByID.Email : '')
    const [phone, setPhone] = useState(empInformationByID.Phone ? empInformationByID.Phone : '')
    const [address, setAddress] = useState(empInformationByID.Address ? empInformationByID.Address : '')

    const [user, setUser] = useState('')

    useEffect(() => {
        setTimeout(() => setUser({ Id: id, Email: email, Phone: phone, Address: address }))
    }, [email, phone, address])
